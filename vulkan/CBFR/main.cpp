...
public:
  ...
  void setWindow(SDL_Window *win)
  {
    ...
    _renderPass = _swapchain->createRenderPass();
  ...
  {
    int count = _swapchain->imageCount();
    _depth = _swapchain->createDepthImage(_w, _h);
    _frameBufs = _swapchain->createFrameBuffer(_renderPass, _depth);
  ...
  void draw()
  {
    ...
    {
      _frame = index;
      auto present = _swapchain->queuePresent(_queue, _frame, _renderSemaphore);
  ...
  void createPipeLayout()
  {
...

    {
      ...
      for (int i = 0; i < 49; i++) {
        mateBufs[i].metallic = (i / 7) / 7.f;
        mateBufs[i].roughness = std::max((i % 7) / 7.f, 0.05f);
        mateBufs[i].ao = 1;
  ...
  void createPipeline()
  {
    ...
    shaderStages[0].module = _device->createShader("pbr.vert.spv");
    shaderStages[0].pName = "main";
    assert(shaderStages[0].module != VK_NULL_HANDLE);

    shaderStages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    shaderStages[1].module = _device->createShader("pbr.frag.spv");
  ...
  void updateUbo()
  {
    ...
    matrixUbo.model = tg::mat4::identity();
