...
DepthPersPipeline::~DepthPersPipeline()
{
  if (_textureLayout)
  {
    vkDestroyDescriptorSetLayout(*_device, _textureLayout, 0);
    _textureLayout = VK_NULL_HANDLE;
  }
}

void DepthPersPipeline::realize(VulkanPass *renderPass, int subpass)
{
  ...
  shaderStages[0].module = _device->createShader(SHADER_DIR "/depth_pers.vert.spv");
  shaderStages[0].pName = "main";
  assert(shaderStages[0].module != VK_NULL_HANDLE);

  shaderStages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
  shaderStages[1].module = _device->createShader(SHADER_DIR "/depth_pers.frag.spv");
...
VkPipelineLayout DepthPersPipeline::pipeLayout()
{
  if (!_pipeLayout) {
...
VkDescriptorSetLayout DepthPersPipeline::textureLayout()
{
  if (!_textureLayout) {
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(*_device, &layoutInfo, nullptr, &texLayout));
    _textureLayout = texLayout;
  }
  return _textureLayout;
}

VkPipelineLayout DepthPersPipeline::createPipeLayout()
{
  VkDescriptorSetLayout desLayout[] = {matrixLayout(), textureLayout()};
