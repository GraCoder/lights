...
{
  if (_shadowMatrixLayout)
  {
    vkDestroyDescriptorSetLayout(*_device, _shadowMatrixLayout, 0);
    _shadowMatrixLayout = VK_NULL_HANDLE;
  }
  if (_shadowTextureLayout)
  {
    vkDestroyDescriptorSetLayout(*_device, _shadowTextureLayout, 0);
    _shadowTextureLayout = VK_NULL_HANDLE;
  }
}
...
void ShadowPipeline::realize(VulkanPass *renderPass, int subpass)
{
  ...
  shaderStages[1].module = _device->createShader(SHADER_DIR "/shadow.frag.spv");
...
{
  if (!_pipeLayout) {
    VkDescriptorSetLayout layouts[] = {matrixLayout(), lightLayout(), pbrLayout(), textureLayout(), shadowMatrixLayout(), shadowTextureLayout()};
...
VkDescriptorSetLayout ShadowPipeline::shadowMatrixLayout()
{
  if (!_shadowMatrixLayout) {
    ...
    _shadowMatrixLayout = deslay;
  }
  return _shadowMatrixLayout;
}

VkDescriptorSetLayout ShadowPipeline::shadowTextureLayout()
{
  if (!_shadowTextureLayout) {
    VkDescriptorSetLayoutBinding layoutBinding = {};
    ...
    VkDescriptorSetLayout deslay = VK_NULL_HANDLE;
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(*_device, &descriptorLayout, nullptr, &deslay));
    _shadowTextureLayout = deslay;
  }
  return _shadowTextureLayout;
...
//}
//
//VkPipelineLayout PerpShadowPipeline::pipeLayout()
//{
