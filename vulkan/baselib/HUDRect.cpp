...
void HUDRect::setGeometry(float x, float y, float w, float h)
{
  ...
  auto dst = _device->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, n, 0);
  auto ori = _device->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, n, vs.data());
  _device->copyBuffer(ori.get(), dst.get(), _device->transferQueue());
...
void HUDRect::fillCommand(VkCommandBuffer cmdbuf, HUDPipeline *pipeline)
{
  vkCmdBindDescriptorSets(cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeLayout(), 0, 1, &_set, 0, 0);
