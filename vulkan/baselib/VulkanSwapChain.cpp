...
void VulkanSwapChain::setSurface(VkSurfaceKHR surface)
{
  ...
  for (int i = 0; i < props.size(); i++)
    vkGetPhysicalDeviceSurfaceSupportKHR(_device->physicalDevice(), i, surface, &supportsPresent[i]);
  ...
  vkGetPhysicalDeviceSurfaceFormatsKHR(_device->physicalDevice(), surface, &formatCount, 0);
  std::vector<VkSurfaceFormatKHR> surfaceFormats(formatCount);
  vkGetPhysicalDeviceSurfaceFormatsKHR(_device->physicalDevice(), surface, &formatCount, surfaceFormats.data());
...
void VulkanSwapChain::realize(uint32_t width, uint32_t height, bool vsync, bool fullscreen)
{
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(_device->physicalDevice(), _surface, &presetModeCount, nullptr);
  std::vector<VkPresentModeKHR> presentModes(presetModeCount);
  vkGetPhysicalDeviceSurfacePresentModesKHR(_device->physicalDevice(), _surface, &presetModeCount, presentModes.data());
...
std::vector<VkFramebuffer> VulkanSwapChain::createFrameBuffer(VkRenderPass vkPass, const VkImageView& depth)
{
  ...
  return createFrameBuffer(vkPass, imgviews, depth);
}

std::vector<VkFramebuffer> VulkanSwapChain::createFrameBuffer(VkRenderPass vkPass, const std::vector<VkImageView>& color, const VkImageView& depth)
{
...
std::tuple<VkResult, uint32_t> VulkanSwapChain::acquireImage(VkSemaphore presentSema)
{
  uint32_t index;
  auto result = vkAcquireNextImageKHR(*_device, _swapChain, UINT64_MAX, presentSema, nullptr, &index);
  return {result, index};
}

VkResult VulkanSwapChain::queuePresent(VkQueue queue, uint32_t index, VkSemaphore waitSema)
{
