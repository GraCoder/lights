...
HUDPipeline::~HUDPipeline()
{
  if (_hudTexLayout)
  {
    vkDestroyDescriptorSetLayout(*_device, _hudTexLayout, 0);
    _hudTexLayout = VK_NULL_HANDLE;
  }
}

void HUDPipeline::realize(VulkanPass *renderPass, int subpass)
{
  ...
  shaderStages[0].module = _device->createShader(SHADER_DIR "/hud.vert.spv");
  shaderStages[0].pName = "main";
  assert(shaderStages[0].module != VK_NULL_HANDLE);

  shaderStages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
  shaderStages[1].module = _device->createShader(SHADER_DIR "/hud.frag.spv");
...
VkDescriptorSetLayout HUDPipeline::textureLayout()
{
  if (!_hudTexLayout) {
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(*_device, &descriptorLayout, 0, &_hudTexLayout));
  }
  return _hudTexLayout;
}

VkPipelineLayout HUDPipeline::createPipeLayout()
{
  VkDescriptorSetLayout setLayout = textureLayout();
