...
VkDescriptorSetLayout TexturePipeline::textureLayout()
{
  if (!_textureLayout) {
...
void TexturePipeline::realize(VulkanPass *renderPass, int subpass)
{
  ...
  shaderStages[0].module = _device->createShader(SHADER_DIR "/texture.vert.spv");
  shaderStages[0].pName = "main";
  assert(shaderStages[0].module != VK_NULL_HANDLE);

  shaderStages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
  shaderStages[1].module = _device->createShader(SHADER_DIR "/texture.frag.spv");
...
VkPipelineLayout TexturePipeline::createPipeLayout()
{
  VkDescriptorSetLayout layouts[] = {matrixLayout(), textureLayout()};
