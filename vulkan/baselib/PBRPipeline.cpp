...
{
  if(_lightLayout) {
    vkDestroyDescriptorSetLayout(*_device, _lightLayout, nullptr);
    _lightLayout = VK_NULL_HANDLE;
  }

  if (_pbrLayout) {
    vkDestroyDescriptorSetLayout(*_device, _pbrLayout, nullptr);
    _pbrLayout = VK_NULL_HANDLE;
  }
}

void PBRPipeline::realize(VulkanPass *renderPass, int subpass)
{
  ...
  shaderStages[0].module = _device->createShader(SHADER_DIR "/pbr_clr.vert.spv");
  shaderStages[0].pName = "main";
  assert(shaderStages[0].module != VK_NULL_HANDLE);

  shaderStages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
  shaderStages[1].module = _device->createShader(SHADER_DIR "/pbr_clr.frag.spv");
...
VkDescriptorSetLayout PBRPipeline::lightLayout()
{
  if (!_lightLayout) {
...
VkDescriptorSetLayout PBRPipeline::pbrLayout()
{
  if(!_pbrLayout) {
    VkDescriptorSetLayoutBinding layoutBinding = {};
...
VkPipelineLayout PBRPipeline::createPipeLayout()
{
  VkDescriptorSetLayout layouts[3] = {matrixLayout(), lightLayout(), pbrLayout()};
